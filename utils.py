...
def to_tensor(in_list):
    return mindspore.Tensor.from_numpy(np.array(in_list))
...
def load_user_info(user_id, dataset='movielens', support_size=16, query_size=4, device: str = "cpu"):
    path = 'data_processed/' + dataset + '/raw/'
    ...
    sup_x1, que_x1 = to_tensor(u_x1[:support_size]), \
                     to_tensor(u_x1[support_size:support_size+query_size])
    sup_x2, que_x2 = to_tensor(u_x2[:support_size]), \
                     to_tensor(u_x2[support_size:support_size+query_size])
    sup_y, que_y = to_tensor(u_y[:support_size]), \
                   to_tensor(u_y[support_size:support_size+query_size])
    sup_y0, que_y0 = to_tensor(u_y0[:support_size]), \
                     to_tensor(u_y0[support_size:support_size+query_size])
...
def get_zeros_like_params(param_list):
    ...
    for param in param_list:
        if count % 2 == 0:
            value = deepcopy(mindspore.ops.ZerosLike()(param.data))
...
def init_params(param_list, init_values):
    ...
    for param in param_list:
        if count % 2 == 0:
            param.set_data(init_values[init_count])
            init_count += 1
        count += 1
...
def init_u_mem_params(param_list, init_values, bias_term, tao):
    ...
    for param in param_list:
        if count % 2 == 0:
            param.set_data(init_values[init_count]-tao*bias_term[init_count])
            init_count += 1
        count += 1
...
def activation_func(name):
    name = name.lower()
    if name == "sigmoid":
        return mindspore.nn.Sigmoid()
    elif name == "tanh":
        return mindspore.nn.Tanh()
    elif name == "relu":
        return mindspore.nn.ReLU()
    elif name == 'softmax':
        return mindspore.nn.Softmax()
    elif name == 'leaky_relu':
        return mindspore.nn.LeakyReLU(0.1)
    else:
...
def mae(ground_truth, test_result):
    if len(ground_truth) > 0:
        pred_y = mindspore.ops.argmax(test_result, axis=1)
        sub = ground_truth-pred_y
        abs_sub = mindspore.ops.abs(sub)
        out = mindspore.ops.ReduceMean(abs_sub.float(), dim=0)
    else:
        out = 1
    return out
